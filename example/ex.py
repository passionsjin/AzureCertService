...
def select_DNS_chall(orderr):
    ...
    raise Exception('DNS01 challenge was not offered by the CA server.')
...
def perform_dns01(client_acme, challb, orderr):
    ...
    print("-> Validation Domain: %s" % challb.chall.validation_domain_name(DOMAIN))
    print("-> Validation Value: %s" % challb.chall.validation(client_acme.net.key))
...
def example_http():
    ...
    # TODO: 여기서부터 DNS challenge로 바꾸기.

    # Select DNS-01 within offered challenges by the CA server
    challb = select_DNS_chall(orderr)
